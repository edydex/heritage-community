...
Source normalization: SyncShow PR #8 is merged into `main` at `590f310`; Multilinguum PR #2 is merged at `9bf201b` after all three checks passed on `daab321`. The Muse documentation correction identifies this repository as public. SyncShow Preview 31 has a separate PR #9 at `c1f9e36`; native packages are still building. Merged branches have not yet been pruned, and unpublished original work remains in the verified private backups.


## Verified delivery and provider checkpoint

SyncShow Preview 31 is published for owner testing in the **private** [preview-builds repository](https://github.com/edydex/heritage-preview-builds/releases/tag/syncshow-v1.4.0-preview.31). All seven installers and ten checksum/provenance/receipt assets were downloaded again and compared byte-for-byte. All nine CI/package jobs passed. PR #9 is merged at `a25d9d1`; the two merged SyncShow branches and the merged Multilinguum branch were removed only after exact ancestry checks, using leases to preserve any concurrent work. Original dirty workspaces remain unchanged. Public distribution gates remain unresolved. The current integration repository is public; its older private-repository labels must not be used to infer access restrictions.

The user-powered original server was reached through its saved LAN identity, and its existing OpenAI key was securely transferred into WOTBC through the supported backup/configuration command. No key was printed or committed. The update succeeded at the same deployed app revisions, both providers are configured, the saved encrypted Muse token still decrypts, and the signed-in manager console shows Start translation enabled.

A bounded rehearsal on WOTBC used the actual deployed Muse adapter for 45 seconds of public-domain English human narration, three seconds of silence, generated note vocabulary, and one Quality English-to-Russian text response. Both providers passed. It did not open a microphone, start a public service or generate speech; it is not physical capture or Russian-source sermon acceptance. The existing Word of Truth sermon fixture upload remains pending fresh user confirmation after automatic approval review rejected it. Total conservatively recorded API testing is $0.91 of $20, with no pending reservation.

Heritage `1f9de27` publishes the audiobook paragraph-navigation feature and reviewed timing data for 54 recordings in five books. Local checks passed: 275 reader unit tests, 124 protocol tests, five matcher cases and five Chromium plus five Firefox audio/navigation tests; the added Confessions data passes the exact paragraph/recording checks. Longer books continue processing locally, and their partial output is not installed. `ba468dc` prepares Android `1.1.40-preview.1` / native code 43; hosted checks and APK publication remain pending. This checkpoint does not claim a new released APK.
