...
Use a dedicated sharing project, stage-specific usage estimates, and an explicit overage policy. The current implementation cannot verify account eligibility, the eligible-model list, or remaining complimentary usage. An administrator must check these in [OpenAI sharing settings](https://platform.openai.com/settings/organization/data-controls/sharing). The earlier audit's model-list and quota notes have not been reverified; do not treat them as a zero-charge guarantee. Economy is now implemented with separate text credentials and defaults to blocked. It requires administrator-confirmed project setup and explicit acceptance of possible billed overage to run. A provider-backed allowance stop remains unfinished.
