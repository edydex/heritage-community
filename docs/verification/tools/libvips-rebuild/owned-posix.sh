...
if [ "$DARWIN" = true ]; then
  ...
  export RUSTUP_HOME="${VIPS_RUSTUP_HOME}"
  mkdir -p $CARGO_HOME
  cp "${VIPS_CARGO_CONFIG}" "$CARGO_HOME/config.toml"
  mkdir -p $RUSTUP_HOME
...
CURL="python3 /private/tmp/heritage-unified-20260912/libvips-rebuild-20260914/retained-fetch.py"

# Download and build dependencies from source

# The pinned Rust and cargo-c tools are supplied by the isolated driver.
...
build_freetype() {
  if [ -d ${DEPS}/freetype ]; then mv ${DEPS}/freetype ${DEPS}/freetype-initial-build; fi
  mkdir ${DEPS}/freetype
