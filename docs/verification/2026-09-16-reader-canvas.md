...
The final Community-only correction makes a text box’s own color override older sermon CSS and dismisses the formatting toolbar when focus leaves the text. The final browser rehearsal includes these checks. Android and the public reader keep the unchanged reader build at `3228063`. WOTBC now runs `8eabb94`. Independent service/public health and backup checks passed. The deployed source files match the tested checkout, and the running browser bundles include the canvas editor, highlighting and final text-color rule. All 11 private recording objects (3,883,891 bytes) have exactly the same inventory checksum before and after deployment; staging is empty. The safety backup is `backup-20260916T225505Z-pre-update`. A transient SMTP DNS verification warning occurred during migration; a lookup in the running app passed afterward. No email was sent.

[SyncShow Preview 30 is published](https://github.com/edydex/heritage-community/releases/tag/syncshow-v1.4.0-preview.30). Every published installer, checksum file and provenance file matches its verified local bytes and size. The release tag points to delivery commit `cc2cc7fc7bd7b3ac30db4e846b3543e93ec16566`. Final Community CI at `8eabb94` passed all four jobs. Physical phone, Windows installation wizard and projector/tablet acceptance remain separate.
