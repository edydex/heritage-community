...
The current Heritage source is `a16045070e938ab7a22d5cfcf0653a388210ab05`. The preceding word-study update adds BSB wording and checked phrase highlights to original-language occurrence results. Greek/BSB mappings now cover matching verses in all 27 NT books, loaded by book; Hebrew/Aramaic results show verse context without claimed word alignment. See Heritage `docs/ORIGINAL-LANGUAGES.md` for source hashes and regeneration. The public reader is updated; this source change does not alter Community server runtime code or require restarting a service. Android 1.1.48-preview.1 (code 51) is published in the normal Latest feed; its [independent receipt](verification/2026-09-21-android-1.1.48.json) verifies all three assets, signer, 697 web files and twelve native tests. The actual checker detects it from earlier versions.
