...
4. LSB/NASB limited slide quotation is permitted subject to publisher terms. Automatic passage import still needs a supported retrieval route that covers retained offline excerpts; the existing NASB95 website integration is a RefTagger popup widget. See Heritage `community-server/docs/BIBLE_PASSAGE_SOURCES.md`. Full-edition imports require an authorized source; a ProPresenter purchase is not treated as a transferable data license. SYNO/UKRK recording permissions and edition identity remain unresolved.
