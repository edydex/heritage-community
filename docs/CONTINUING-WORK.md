...
The current Heritage source is `5e3bc8e4a2901085648e7fa34bbc4e9547b505db`. The preceding word-study update adds BSB wording and checked phrase highlights to original-language occurrence results. Greek/BSB mappings now cover matching verses in all 27 NT books, loaded by book; Hebrew/Aramaic results show verse context without claimed word alignment. See Heritage `docs/ORIGINAL-LANGUAGES.md` for source hashes and regeneration. The public reader is updated; this source change does not alter Community server runtime code or require restarting a service. Android 1.1.45-preview.1 (code 48) is published in the normal Latest feed; its [independent receipt](verification/2026-09-21-android-1.1.45.json) verifies all three assets, signer, 694 web files and eleven native tests. The actual checker detects it from earlier versions.
