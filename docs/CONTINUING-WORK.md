...
The current Heritage source is `1d57fe30a4cac26f95450d928e2acbb33bec4743`. The preceding word-study update adds BSB wording and checked phrase highlights to original-language occurrence results. Greek/BSB mappings now cover matching verses in all 27 NT books, loaded by book; Hebrew/Aramaic results show verse context without claimed word alignment. See Heritage `docs/ORIGINAL-LANGUAGES.md` for source hashes and regeneration. The public reader is updated; this source change does not alter Community server runtime code or require restarting a service. Android 1.1.47-preview.1 (code 50) is published in the normal Latest feed; its [independent receipt](verification/2026-09-21-android-1.1.47.json) verifies all three assets, signer, 697 web files and eleven native tests. The actual checker detects it from earlier versions.
