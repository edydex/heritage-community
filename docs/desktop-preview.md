...
Preview 31 adds **Monochrome** in tablet teaching, with named patterns and a used-color reference while congregation colors stay unchanged. Authorized Bible editions can be installed through **Prepare → This computer → Scripture → Bible translations**; imported text and credits travel with offline service packages. WOTBC currently has both provider credentials configured and the Quality start control enabled; the fresh real-sermon provider check is pending separate upload approval.
