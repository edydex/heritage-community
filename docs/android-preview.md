...
In the installed Android app, open **Settings → Advanced Settings** and check for updates. [Heritage 1.1.37-preview.1](https://github.com/edydex/heritage_study_bible/releases/tag/v1.1.37-preview.1) is now the normal GitHub Latest release, and the installed checker detects it from earlier builds through 1.1.36. Version code 40 preserves the existing application identity and signer.

You can also download `heritage-study-bible-1.1.37-preview.1-debug.apk` from that release. Install it over the existing Heritage app; keep the previous installation and its data until the phone rehearsal passes. Do not uninstall first. The release check requires the same application ID and signer as v1.1.32 and a higher Android version code. This release increases the numeric version, so the older checker can detect it too.
...
[Current build, published assets and acceptance evidence](verification/2026-09-16-sharing-events.md). Earlier [Automatic Sync native evidence](verification/2026-09-14-automatic-sync.json) remains available.
