...
Fresh setup runs the existing Community wizard for church/admin settings, optional email and tunnel, and scheduled backups. It then offers translation-provider setup. Quality/Economy captions and optional speech use the existing Community connection; a separate LiveKit account is not required. Optional relay setup is for direct Realtime audio. Quality/Economy services do not contact that relay even when its credentials are stored. Enter secrets in the server wizard; the unified command does not forward your local environment. The existing installer may send its configured administrator sign-in test when email authentication is enabled.
