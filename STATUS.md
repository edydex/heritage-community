...
See the [API-test ledger](docs/verification/api-test-budget.json) for usage against the **$20 maximum**, and [release-input record](docs/verification/2026-09-13-native-release-inputs.md) for the exact remaining desktop gates. The verified Preview 28 QA installer is retained locally; it is separate from the official release. A [Mac FFmpeg source-replacement rehearsal](docs/verification/2026-09-14-ffmpeg-source-replacement.md) now verifies a rebuild from matching FFmpeg/Chromium Opus sources, AAC/Opus/H.264 decoding, and loading in a freshly extracted, re-signed Preview 28 copy. All original exported names and the selected codec configuration are retained; image output is unchanged. libvips/Canvas, GUI media, other-target and release-input checks remain open.
...
A [Mac Canvas wrapper rebuild](docs/verification/2026-09-14-canvas-wrapper-rebuild.md) now retains Rust 1.97.1, a complete owned Cargo lockfile and 93 vendored registry packages. Its signed replacement preserves the preview’s English/Russian text, image and PDF outputs. An initial incompatible N-API resolution was caught by actual rendering; the published packaging verifier now checks that image round-trip. The subsequent [Skia source rebuild](docs/verification/2026-09-14-skia-source-rebuild.md) replaces those upstream static libraries with an owned build from pinned Skia and dependency sources. Its signed Mac copy also passes the same rendering and packaged-runtime checks. Other platforms, source/notice distribution, libvips and official-release requirements remain open; the compatible-set pins are unchanged.
