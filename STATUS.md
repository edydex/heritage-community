...
| Personal notes and progress | Real email sign-in and two-way note/reading-position transfer passed between Firefox and the internal browser, including conflict review. A fresh sync also completed at 10:46 Pacific. | [Personal-sync acceptance](docs/verification/2026-09-13-personal-sync.md) |
...
- **Android Community preview:** the APK and its three new Community/native-storage tests passed an earlier emulator run, but visual inspection found a launcher dialog covering the app. The current run requires the expected screen to be visibly present and corrects the prebuild scope. The APK is not yet published. [Current runner and verification record](docs/verification/2026-09-13-android-runner.md).
